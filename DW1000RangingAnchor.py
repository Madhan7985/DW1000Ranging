...
#    host = '10.2.128.64'  #socket.gethostname()                           
#    port = 9999
...
#To create A server
    ...
    host = '10.2.129.175'#socket.gethostname()                           
    port = 8888
    # bind to the port
...
#        t = time.time()
        ...
        for currentTag in tagList:
            while True:
                success, distance = loop(currentTag)
                if(success):
                        break
            allDistances.append(distance)
        print allDistances
        clientsocket.send(", ".join( repr(e) for e in allDistances) )
